...
class CubeSphereAccessor(object):
    ...
    def plot_CSlayer(self,ax=None,vmin=None,vmax=None,transform=ccrs.PlateCarree(),cbar=True):
        ...
        if vmax is None: vmax=self._obj.max()
        if vmin is None: vmin=self._obj.min()
    ...
    def meridional_mean(self,dlon=5,verbose=False):
        ...
        if verbose: print("Dimension of each group:",list(group)[0][1].dims )
...
def open_FV3data(dirname,prefix,verbose=False):
    ...
    for i in range(6):
        ...
        if verbose: print("opening",filename,end=", ")
